    ...
    for timesignal in results:
        ...
        if run <= accumulations:
            if run == 1:
                ...
                accuf = Accumulation(error=False)
            if (((run-1)%2)==0):
                accu += timesignal
                accuf += filtered
            elif (((run-1)%2)==1):
            ...
            if run == accumulations:
                ...
                sqrt = (numpy.sqrt(accuf.y[0][r_start:r_end]**2 + accuf.y[1][r_start:r_end]**2).mean() - numpy.sqrt(accuf.y[0][-1024:]**2 + accuf.y[1][-1024:]**2).mean())/run
