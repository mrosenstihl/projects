...
def Hahn_exp(pulse90, pulse180, gradient, delta, conv, tau, run, accumulations, td, rec_phase):
    ...
    e.record(samples=4*4096, frequency=1e6, sensitivity=5)
    return e
...
def experiment():
    ...
    rec_phase = 100
    gradient_list = linspace(0,9,10)
    #tau_list=[12e-3,14e-3,16e-3,18e-3]
    tau_list = log_range(3e-3,10e-3, 4)
    accumulations      = 8    # number of repetitions (for a given echo delay), 8 for full phase cycle
